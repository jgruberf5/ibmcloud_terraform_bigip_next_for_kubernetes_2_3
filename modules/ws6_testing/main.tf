...
locals {
  ...
  cluster_zones = (var.testing_create_cluster_jumphosts && !var.create_roks_cluster) ? toset(data.ibm_is_zones.cluster_region_zones[0].zones) : toset([])
  ...
  # Skip when create_roks_cluster=true: the cluster data source is deferred to
  # apply time, making the VPC ID unknown, which prevents for_each key derivation.
  # A second `terraform apply` once the cluster exists will add the attachments.
